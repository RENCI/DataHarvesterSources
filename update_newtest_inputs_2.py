...
def process_contrails_stations(time_range, contrails_stations, product_type, metadata, config):
    # Fetch the data
    try:
        contrails = contrails_fetch_data(contrails_stations, time_range, config, product=product_type, owner='NCEM')
...
def main(args):
    ...
    # Now build urls
    # NOWCASTS
    nowfrmt='http://tds.renci.org/thredds/dodsC/%s/nam/%s/hsofs/hatteras.renci.org/hsofs-nam-bob-2021/nowcast/fort.61.nc'
    ...
    #FORECASTS
    forefrmt='http://tds.renci.org/thredds/dodsC/%s/nam/%s/hsofs/hatteras.renci.org/hsofs-nam-bob-2021/namforecast/fort.61.nc'   
...
# def __init__(self, station_id_list, periods, config, product='river_water_level', owner='NCEM'):
    ...
    process_contrails_stations(time_range, contrails_stations_rivers, 'river_water_level', contrails_river_metadata, contrails_config)

    # Coastal
    contrails_coastal_metadata='_COASTAL_'+endtime.replace(' ','T') # +'_'+starttime.replace(' ','T')
    process_contrails_stations(time_range, contrails_stations_coastal, 'coastal_water_level', contrails_coastal_metadata, contrails_config)
